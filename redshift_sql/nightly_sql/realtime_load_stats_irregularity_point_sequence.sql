update {{ dw_schema_name }}.stage_irregularity_jam_point_sequence_{{ batchIdValue }}  set elt_run_id={{ batchIdValue }};
...
SELECT stg.elt_run_id,
       (select table_id from {{ elt_schema_name }}.DW_TBL_INFO where TABLE_NAME ilike 'irregularity_jam_point_sequence') table_id,
       'irregularity_jam_point_sequence' AS TABLE_NAME,
       stg.ingested_rows
FROM   (SELECT elt_run_id,
               Count(*) INGESTED_ROWS
        FROM   {{ dw_schema_name }}.stage_irregularity_jam_point_sequence_{{ batchIdValue }} tfi
