-------------------------------------------------------------------
--Temp Logic Begin --- This is temporary fix 
-------------------------------------------------------------------
INSERT INTO dw_waze.rare_alert
SELECT 
dwa.alert_char_crc ,
  dwa.alert_uuid          ,
  dwa.uuid_version         ,
  ...
  dwa.road_type
FROM dw_waze.alert dwa
  JOIN (SELECT alert_uuid,
               alert_char_crc
        FROM (SELECT alert_uuid,
                     alert_char_crc,
                     COUNT(*) OVER (PARTITION BY alert_char_crc,alert_uuid) count_test
              FROM dw_waze.alert
              WHERE etl_current_flag = 1
              ORDER BY alert_char_crc)
        WHERE count_test > 1
        GROUP BY alert_uuid,
                 alert_char_crc) FILTER on filter.alert_uuid=dwa.alert_uuid
                 AND filter.alert_char_crc=dwa.alert_char_crc;

DELETE FROM dw_waze.alert USING
(SELECT alert_uuid,
               alert_char_crc
        FROM (SELECT alert_uuid,
            alert_char_crc,
...
WHERE count_test > 1
GROUP BY alert_uuid,
         alert_char_crc) FILTER WHERE FILTER.alert_uuid = dw_waze.alert.alert_uuid AND FILTER.alert_char_crc = dw_waze.alert.alert_char_crc
;

-------------------------------------------------------------------
--Temp Logic END - This is a temporary fix
-------------------------------------------------------------------
...
over ( 
  PARTITION BY 
tfi.alert_uuid, tfi.alert_type, tfi.num_thumbsup, tfi.reliability, tfi.confidence, tfi.report_rating, tfi.location_lat, tfi.location_lon, tfi.pub_millis, tfi.pub_utc_timestamp, tfi.state) total_counts,
       ...
       AND ( Min(tfi.pub_millis::BIGINT) 
               over ( 
                 PARTITION BY tfi.alert_uuid) < Min(dwai.start_pub_millis::BIGINT) 
                                                  over ( 
                                                    PARTITION BY 
           tfi.alert_uuid) OR tfi.pub_millis::BIGINT <Max(dwai.pub_millis::BIGINT)over ( 
                                                    PARTITION BY 
 ...
 GROUP  BY revision_flag, 
     ...
     AS (SELECT CASE 
                ...
                CASE 
                  ...
                  ORDER BY 
                  stg_trnsform.pub_millis::BIGINT, stg_trnsform.state) 
                  WHEN fa.alert_uuid IS NOT NULL 
...
SELECT alert_char_crc,
       alert_uuid,
       ROW_NUMBER() OVER (PARTITION BY rvs.alert_uuid ORDER BY rvs.pub_millis::BIGINT, rvs.state) uuid_version,
       CASE
         WHEN MAX(rvs.pub_millis::BIGINT) OVER (PARTITION BY rvs.alert_uuid) = rvs.pub_millis::BIGINT THEN 1
         ELSE 0
       END current_flag,
       etl_run_id,
       etl_current_flag,
