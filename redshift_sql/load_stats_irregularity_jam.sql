...
SELECT stg.elt_run_id,
       (select table_id from {{ elt_schema_name }}.DW_TBL_INFO where TABLE_NAME ilike 'irregularity_jam') table_id,
       'irregularity_jam' AS TABLE_NAME,
       stg.ingested_rows, 
       prs.persisted_rows 
FROM   (SELECT elt_run_id, 
               Count(*) INGESTED_ROWS 
        FROM   {{ dw_schema_name }}.stage_irregularity_jam_{{ batchIdValue }} tfi
        GROUP  BY elt_run_id) stg 
       JOIN (SELECT elt_run_id, 
                    Count(*) PERSISTED_ROWS 
             FROM   {{ dw_schema_name }}.int_irregularity_jam_{{ batchIdValue }} tfi
             GROUP  BY elt_run_id) prs 
         ON stg.elt_run_id = prs.elt_run_id ;

DROP TABLE IF EXISTS stage_irregularity_jam_{{ batchIdValue }};
DROP TABLE IF EXISTS int_irregularity_jam_{{ batchIdValue }};
